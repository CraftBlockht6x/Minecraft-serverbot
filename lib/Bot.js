...
async function triggerAIResponse(source, message, isWhisper = false, whisperTarget = null) {
  if (!cfg.DEEPSEEK_API_KEY) {
    localLog('[AI] 未配置 DEEPSEEK_API_KEY,跳过');
    return;
  }
  ...
  const keywords = getKeywords();
  localLog(`[AI] 触发检查: 模式=${cfg.AI_TRIGGER}, 私聊=${isWhisper}, 提到=${mentioned}, 关键词命中=${keywordMatched}, 关键词=[${keywords.join(', ')}], 来源=${source}`);

  // 收到私聊时强制命中 AI,否则按 AI_TRIGGER 配置判断
  let shouldTrigger = isWhisper;
  if (!shouldTrigger && cfg.AI_TRIGGER === 'none') {
    localLog('[AI] AI_TRIGGER=none,跳过回复');
    return;
  }
  if (!shouldTrigger) {
    if (cfg.AI_TRIGGER === 'all') {
      ...
      localLog(`[AI] 未知触发模式 ${cfg.AI_TRIGGER},跳过`);
      return;
    }
  }

  if (!shouldTrigger) {
    localLog('[AI] 未满足触发条件,跳过回复');
    return;
  }
...
function createBot() {
  ...
  client.on('text', (packet) => {
    ...
    if (isSelfEcho(msg)) {
      localDebug('[AI] 检测到假人自己发送内容的回显,跳过处理');
      return;
    }
